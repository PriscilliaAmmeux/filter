...
function App() {
  const [simpsonsOnly, setSimpsonsOnly] = React.useState(false);
  ...
  function show() {
    setSimpsonsOnly(!simpsonsOnly);
  }
  const funcFilter = (person) =>
    !simpsonsOnly || person.character.includes('Simpson');

  return (
    <div>
      <button onClick={show}>Family's Simpson</button>
      {quoteList.filter(funcFilter).map((quote, index) => (
